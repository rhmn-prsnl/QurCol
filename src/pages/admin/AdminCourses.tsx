...
export default function AdminCourses() {
  ...
  const handleAddCourse = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsAdding(false);
      setNewCourse({ title: '', description: '', category: '', level: '', instructor: '', thumbnail: '' });
      fetchCourses();
    }, 500);
  };
    ...
    <div>
      ...
      {isAdding && (
        <div className="bg-zinc-50 dark:bg-zinc-900 p-6 rounded-2xl shadow-sm border border-gold-200/50 dark:border-gold-900/30 mb-8">
          <h2 className="text-lg font-bold text-black dark:text-white mb-4">Add New Course</h2>
              ...
              {courses.map(course => (
                <tr key={course.id} className="bg-zinc-50 border-b border-gold-100 dark:bg-zinc-900 dark:border-gold-900/20 hover:bg-white dark:hover:bg-black transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    ...
                    <button onClick={() => handleDelete(course.id)} className="text-red-600 dark:text-red-400 hover:underline" title="Delete">
